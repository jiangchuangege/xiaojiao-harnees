...
# 默认替代建议：一律指向"新建 / 用 edit_file 改一段 / 手动处理"这条唯一出路。
# ⚠️ **不要再写 `mode='a'`（2026-09-21 改）**：`write_file` 压根没有 mode 参数，
#    模型照着这条建议**做不了**，只会重试原动作 —— 实测用户连着三轮被同一段拦截刷屏、
#    一句代码都没拿到（载体后来改成"替它换个新名字写进去"，见**主程序**里的
#    `_alt_write_on_overwrite` —— 这里不写主程序的文件名，本模块不许跟它耦合，自测 [F2] 盯着这条）。
_DEFAULT_ADVICE = ("改成新建（换个新文件名）或用 edit_file 只改其中一段；"
