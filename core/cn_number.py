...
【这段为什么这么设计】
    ...
    为什么不让 4B 去翻译：4B 对"两万三""一千二"这类**口语省略写法**非常不稳，
    实测会把"两万三"翻成 2300 或 20300（漏一位/多一位），而用户是拿它去算钱的。
    所以：规则覆盖的写法一律走规则（零延迟、必然正确）；规则覆盖不到的极端写法，
    才让 4B 给一个**纯数字表达式**，而且**必须过载体的格式校验**才采信。
