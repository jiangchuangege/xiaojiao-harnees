...
CARRIER_MARKS = (
    ...
    "已被载体层拦截",
    "安全红线",
    # 【机器专用标记（2026-09-21 加）】载体想让结果算"载体动作"、但**又不想让用户看见那句警示**时用它：
    # 展示前会被剥掉（见主程序的 `_carrier_block_answer`）。加它的直接原因：用户连着三次被
    # 「⚠️ 该操作被载体层拦截（安全红线）」刷屏，而他要的只是"让我看看代码" —— 那一拦是载体在
    # 正常工作，不该长成一条告警横幅；但它**必须**能让健康系统认出来，否则会被算成 `tool_misuse`
    # → 诊断 HEAVY → 三级治疗换火种 → 去慢盘重载 4GB → 用户看到「一轮十分钟不动」（实测）。
    "【载体动作】",
)
