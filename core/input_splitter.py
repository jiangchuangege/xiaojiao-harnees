...
def split_input(text, max_chunk=DEFAULT_MAX_CHUNK):
    ...
    【为什么这么设计】"输入无限"的物理前提是：单次请求装不下任意长的输入。
    既然装不下就必须切开，而**切法直接决定结果质量**：
    切在句中会把一句话劈成两半，两片各自被处理时都得到残缺的意思，
    拼出来的结论就是错的（而用户只会觉得"它总结得不对"，查不出原因）。
    所以这里坚持按段落边界切，只有超长段落才降级到句子边界。

    【去掉它会怎样】长输入只能被硬截断（用户贴 10 万字，系统只用前 5000 字），
    或者整段扔给模型换回一个"exceeds context"报错 ——
    前者是**静默丢内容**（最坏：用户根本不知道自己贴的东西被丢了），后者是撞墙。
    """
    text = text or ""
