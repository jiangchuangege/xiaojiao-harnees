...
class ScraplingBridge:
    ...
    def get_tool_descriptions(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            T("make_request", "抓普通网页的官方同名工具(纯HTTP，最快)。什么时候用：和 get 完全等价，脚本/老习惯用这个名；新写法直接用 get。输入 url；输出 页面正文+HTTP状态",
              {"url": S_URL, "timeout": S_TO, "save_to": S_SAVE, "ignore_robots": S_IGN}, ["url"]),
            T("open_session", "开浏览器会话，用于登录态/过验证后再抓。什么时候用：目标页要登录或要先过 Cloudflare 挑战；输入 session_type(dynamic/stealthy)、可选 session_id；输出 会话ID(拿去 session_fetch，用完务必 close_session)",
              {"session_type": S_STYPE, "session_id": S_SID}, []),
            T("open_request_session", "开纯 HTTP 会话(保持 cookie，不开浏览器)。什么时候用：要连续请求同一站点且需带 cookie；输入 可选 session_id；输出 会话ID(拿去 session_make_request)",
              {"session_id": S_SID}, []),
            T("close_session", "关闭会话、释放资源。什么时候用：session_fetch/screenshot 做完立刻关，别让会话堆着；输入 session_id(必填)；输出 关闭结果", {"session_id": S_SID}, ["session_id"]),
            T("list_sessions", "查现在还有哪些会话开着。什么时候用：排障、确认自己有没有忘记 close_session；无参数；输出 会话列表", {}, []),
            T("session_fetch", "在**已开浏览器会话**里抓页面(保持登录态/已过验证)。什么时候用：先 open_session 再抓；输入 url + session_id；输出 页面正文",
              {"url": S_URL, "session_id": S_SID, "wait_selector": S_WAIT, "ignore_robots": S_IGN},
              ["url", "session_id"]),
            T("session_make_request", "在**已开 HTTP 会话**里发请求(保持 cookie)。什么时候用：open_request_session 之后；输入 url + session_id；输出 响应正文",
              {"url": S_URL, "session_id": S_SID, "ignore_robots": S_IGN}, ["url", "session_id"]),
            T("screenshot", "给页面截图并存到本地。什么时候用：需要页面长什么样/留证据；输入 url + session_id，可选 full_page 整页；输出 图片路径(media/screenshot/)",
              {"url": S_URL, "session_id": S_SID, "full_page": S_FULL, "ignore_robots": S_IGN},
              ["url", "session_id"]),
            # ---------- 小焦增强（原生没有 / 更好用）----------
            T("get", "**抓网页首选**(纯HTTP，最快)。什么时候用：静态页/接口/JSON 一律先用它；输入 url(可选 stealth/timeout/save_to)；输出 页面正文+HTTP状态。被拦或正文空 → 再依次换 fetch → stealthy_fetch",
              {"url": S_URL, "stealth": S_STEALTH, "timeout": S_TO, "save_to": S_SAVE, "ignore_robots": S_IGN}, ["url"]),
            T("bulk_get", "一次抓多个**静态**网页(比逐个 get 快)。什么时候用：给了多个 URL 且都是普通页；输入 urls 数组；输出 每个 URL 的正文(保序，单个失败不影响其它)",
              {"urls": S_URLS, "stealth": S_STEALTH, "ignore_robots": S_IGN}, ["urls"]),
            T("fetch", "**需要浏览器渲染**时用(JS 动态页/懒加载)。什么时候用：get 只拿到空壳或内容不全；输入 url(可选 wait_selector 等元素出现)；输出 渲染后的正文。比 get 慢，能不用就不用",
              {"url": S_URL, "wait_selector": S_WAIT, "timeout": S_TO, "save_to": S_SAVE, "ignore_robots": S_IGN}, ["url"]),
            T("bulk_fetch", "批量用浏览器渲染抓取。什么时候用：多个页面都需渲染；输入 urls；输出 各页正文(较慢)",
              {"urls": S_URLS, "ignore_robots": S_IGN}, ["urls"]),
            T("stealthy_fetch", "**隐身抓取，专门绕 Cloudflare/风控**。什么时候用：get 和 fetch 都失败(403/挑战页)才用，开销最大；输入 url；输出 正文",
              {"url": S_URL, "timeout": S_TO, "save_to": S_SAVE, "ignore_robots": S_IGN}, ["url"]),
            T("bulk_stealthy_fetch", "批量隐身抓取。什么时候用：多个都被风控拦的页面；输入 urls(≤20)；输出 各页正文(很慢，慎用)",
              {"urls": S_URLS, "ignore_robots": S_IGN}, ["urls"]),
            T("scrape_with_selector", "只取页面**指定区块**(CSS 选择器)，且自适应抗改版。什么时候用：只要某一块(正文/列表)或站点常改版；输入 url + selector；输出 匹配到的内容",
              {"url": S_URL, "selector": S_SEL, "adaptive": S_ADAPT, "name": S_NAME, "ignore_robots": S_IGN},
              ["url", "selector"]),
            T("browser_session", "会话+截图的**聚合入口**(一个工具走完 开→抓→截→关)。什么时候用：不想分步调 open/session_fetch/close；输入 action(open/open_http/close/list/fetch/request/screenshot) 等；输出 对应结果",
              {"action": {"type": "string",
               ...
               "ignore_robots": S_IGN},
              ["action"]),
            T("download", "下载**文件**(PDF/EPUB/ZIP/图片/音视频)到本地。什么时候用：URL 指向文件而不是网页；输入 url(可选 filename)；输出 本地文件路径",
              {"url": S_URL, "filename": S_FN, "ignore_robots": S_IGN}, ["url"]),
            T("collect_vulnerabilities", "**漏洞清单唯一入口**(NVD 结构化数据)。什么时候用：用户要「最近的漏洞/CVE/高危漏洞」；输入 days(默认7,≤120)、severity(HIGH 默认=含以上)、limit(默认5)；输出 Markdown 漏洞表(编号/等级/评分/受影响软件/时间/摘要)。不要用 web_search 凑漏洞清单",
