...
def run(res: Results) -> Results:
    ...
    for raw, want in (("用搜索工具找漏洞", "漏洞 CVE"),
                      # ⚠️ Bug 2 修复后这里从 "最近的漏洞 CVE" 变成 "最近漏洞 CVE"（少一个"的"）。
                      #    为什么**改期望而不是改代码**：本文件下面那条"主题词提取"已经写明
                      #    `_query_tokens("最近的漏洞 CVE") == ["漏洞","CVE"]` ——
                      #    也就是载体自己的相关度打分**本来就把"的"当噪声**，组装变体时也会剥掉。
                      #    留着"的"只是把噪声推给下游；现在在提取阶段就去掉，得到的变体更干净：
                      #    ['最近漏洞 CVE', '漏洞 CVE', '漏洞 CVE 最近']。
                      ("用联网搜一下最近的漏洞", "最近漏洞 CVE"),
