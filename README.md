...
**长文本编码偏向尾部，但不影响最终结果。** 末块加权保证尾部不被稀释，代价是长记忆的向量主要由结尾决定。这一偏向**只影响召回集合的偏移**：小脑只负责把可能相关的记忆捞回来，真正的相关性判断由 4B 大脑精排（core/retriever.py 的 rerank）完成，它读的是完整正文。**召回端允许偏，判断端不偏。** 分块编码由 core/embedder.py 的 _CHUNK_ENABLED 控制，默认开启（短文本不触发，零影响）。
