...
| **状态真的改变行为** | 代码层硬改输入 / 行动 / 主动，判据是"这一轮的工具表真的短了" |
| **互联网是它的世界，不是工具箱** | 它在里面看、走、学、记、想、动 |
| **可以自主做事，但不能删任何文件** | 载体层硬拦截，与权限开关无关 |
| **模型会退化，医生能自愈** | 18 类症状 → 四级诊断 → 四级治疗 → 病历 → 预防 |
| **载体给模型叠加等效精度，4B ≥ 360B** | 360B 是存量精度（一次用完），4B + 载体是流量精度（可无限叠） |
| **全程本地、离线可用** | 模型、记忆、会话都在你的机器上 |
| **全量测试 248 / 249 · 通过率 100%** | 1 项按当天 NVD 数据跳过，非失败 |

深入阅读 → [`docs/design-philosophy.md`](docs/design-philosophy.md)（世界观与工程依据）·
[`docs/一条没人走过的路.md`](docs/一条没人走过的路.md)（项目手记，含失败与未通的那一跳）
