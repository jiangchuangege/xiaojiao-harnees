...
用户回来（relieved=True）           → 低沉开始散，但一次散不干净（抑郁了散得慢）
floor()                            → 底 < 0（与"回到 0"分界）
```
...
3. **抑郁不等于临床意义上的抑郁**。这里是"底色被压低了"这个**机制**的工程名字，
   不宣称任何医学含义 —— 不许拿它当心理诊断。
4. **「孤独 / 低沉 / 抑郁」这三个词只在载体内部和本文档里，不进注入文本。**
   2026-09-18 起 `core/inner.py` 的 `render()` 改成**只列事实与数字**：
   `多久没人说话 / 空闲 / 载体刻度（原始字段值）/ 它自己说过 / 它自己认下 / 偏向（来自…）`，
   空的时候返回空串（不硬凑）。原因是用户定的口径：**载体写好词句 = 假，模型自己感知 = 真** ——
   把这三个词摆给它，等于替它下结论。刻度照旧算，只是不再翻译成形容词。
