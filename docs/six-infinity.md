...
| ⑦ | 上下文无限 | **对话可以一直聊下去，不撞窗口**：不扩 ctx，改「装配什么进去」—— 最近几轮留原文、更早的压成摘要、再往前的靠记忆库按需检索。 |

关键区分在三处：

- 记忆无限的关键词是「全部」和「永久」：不是「最近 10 轮摘要」，也不是「模型自己记着」。模型上下文里只有这一轮需要的那几条，但库里存着从第一天到现在的所有对话。
- **上下文无限管的是"这一轮装什么进去"**：它把"更老的历史"压成摘要、只留最近几轮原文（`HISTORY_KEEP_FULL = 4`），
  所以窗口不被对话长度撑爆。它与记忆无限的分工是：**记忆无限保证"记得住"，上下文无限保证"聊得下去"。**
- 单次永不超不做任何「无限」承诺：装不下时如实报错，绝不静默丢内容。

> 上下文无限的完整原理（四层防护、ctx 校准 20000→20224、token 估算为什么是 1.5 字/token、
> 摘要怎么压）见 [`context-infinity.md`](context-infinity.md)。
...
| 模块 | 职责 | 关键实现 |
| --- | --- | --- |
| `core/embedder.py` | 文本 → 512 维单位向量 | 主后端：小脑 MiniGPT 过完整个编码器栈（embedding + pos → 8 层 Transformer，**双向注意力、无因果掩码**）→ 长文本按 96 字切块、逐块池化后末块加权融合（短文本不分块）→ 加 α 公共方向 → 加公共方向标定 → L2 归一化；兜底：字符 2/3-gram 哈希向量（同为 512 维） |
...
| `RERANK_GAP` | 0.50 | top1 与 top2 的分差 ≥ 0.50 视为「top1 真的碾压」，跳过精排 |

分差闸门的依据：top1 明显领先时向量排序本身可信；top1 与 top2 咬得很近时，正是反义或无关记忆混进来的典型形状（实测「我喜欢蓝色」0.90 对「我讨厌蓝色」0.88，分差只有 0.02），必须让大脑裁一下。

> 2026-09-18 这条线由 `0.10` 提到 `0.50`：`0.10` 太容易过，真实查询里 top1 领先 0.2 是家常便饭，于是绝大多数查询都走「排名不含糊 → 跳过精排」，**精排实际上很少跑**（实测日志：「3 条候选 → top1 领先 0.236，排名不含糊 → 跳过精排」）。代价是精排调用变多（每次约 400ms）—— 明知代价、选择精度。

**失败分两种处置（2026-09-18 起不再笼统说「失败一律放行」）**：

- **判官不可用**（大脑没起 / 报错 / 返回空）→ **不全部放行**，改按 `THRESHOLD = 0.6` 卡一道；卡完若一条不剩，仍保留 top1（不空手而归）。原来这条路是"全部放行"，于是只要大脑这条线不通，噪音就全进 system（实测日志：「3 条候选 → 判官不可用，全部保留」，游泳 / 周杰伦照样进去）。**这不是载体替模型判断** —— 判官在时照常问大脑，判官不在时载体手里只剩已算出的分数，按分数卡一道不叫替它判断，叫"没有判断可问的时候，不许把噪音当事实"。
- **判官可用、只是判为「无」**、以及**解析不出编号** → 仍保守保留全部候选。理由是小模型偶尔会把全部候选判成无关，真按它说的清空，用户会觉得「它什么都不记得了」，那种观感比多注入一条更差。
- **`rerank()` 自身抛异常** → 最外层 `except` 依旧**全部放行**（这一条没改，如实标注）。
...
3. **写入路径存了整段回答** → 长文本按 96 字切块、逐块池化后末块加权融合（短文本不分块）→ 加 α 公共方向按长度加权，用户那句「我叫张三」被几百字寒暄淹掉，cos 掉到阈值以下、注入 0 条。修法即 4.1.5 的 `key_text`。
...
每张图都可以脱离上下文阅读：图题、一句话说明、代码位置索引齐全。

**全仓统一配色约定**（这一套是**全仓库唯一**的一套）：

| 色值 | 什么时候用 |
|---|---|
| `#4A90E2` | 主色 · 过程节点（载体在做的事） |
| `#7ED321` | 正常 · 已完成 · 结果（"这条通了"） |
| `#F5A623` | 部分落地 · 注意 · 待判断 |
| `#E74C3C` | 未落地 · 风险 · 红线 · **已废弃的旧做法** |
| `#8C8C8C` | 中性 · 不在场 · 不参与 |

浅色底也统一：`#EAF2FD`（蓝）/ `#EAF7E2`（绿）/ `#FEF6E7`（橙）。

> 早期有两套色在混用（另一套是 `#2d6cdf` / `#5cb85c` / `#f0ad4e` / `#d9534f`），
> 同一册图看起来不像一个人画的。现已**全仓统一到上表这一套**（396 处色值）。
> 时序图没有节点 style，改用 init 里的 `themeVariables` 对齐同一批色值。
...
flowchart TB
    ...
    W --> R
    N50 -.->|"库在长，上下文不变"| R
    R --> RR{"top1 与 top2<br/>分差小于 0.50"}
    RR -->|"是"| RJ["载体二次判断<br/>大脑从候选里挑真相关的<br/>判官不可用 → 按 0.6 卡一道（至少留 top1）"]
    RR -->|"否"| INJ
...
对话能一直聊下去，靠的不是把窗口撑大，而是**四层防护**（完整原理见 [`context-infinity.md`](context-infinity.md)）：
...
flowchart TB
    ...
    G --> H
    I["⚠️ reserve 必须算上 tools schema<br/>77 个工具的 JSON 是几千 token<br/>漏算它 → 裁完仍然超限"] -.-> E
    style A fill:#4A90E2,color:#fff
    style B fill:#4A90E2,color:#fff
    style C fill:#4A90E2,color:#fff
...
**这张图要说的一句话**：窗口装不下是物理事实，改不了；能改的只有**装配顺序** ——
先保 system 与本轮问题，再从最老的历史开始丢，丢掉的不直接扔而是压成摘要。
**记忆无限保证"记得住"，上下文无限保证"聊得下去"，单次永不超保证"发得出去"。**

### 5.8 详细流程图索引

| 文件 | 内容 |
| --- | --- |
| [01-overview.md](six-infinity-diagrams/01-overview.md) | 七个无限总览图 |
...
这是载体优先最实际的价值。七个无限的实现全部在 `core/` 与 `xiaojiao_app.py` 里，不在模型里。换大脑（本地 4B、编码 8B、云端 API）时：
...
按方案共 6 项：① 全量 249/249 ｜ ② 七个无限逐项 ｜ ③ 跨步骤衔接 ｜ ④ 连续 200 轮长跑 ｜ ⑤ 多模型（本地 4B + 云端）｜ ⑥ 极端（贴 50 万字、写 20 万字、连问 500 轮）。
...
七个无限里只有⑥站在物理上限这一侧（另外五条都是载体在模型外围兜出来的，不对物理上限做任何承诺）。载体能做的是精确按需装配、切片、多次请求，不是让单次变大。两点要记住：
