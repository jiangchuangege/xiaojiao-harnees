...
| 8 | ruff 全量规则集未清零 | 实测 `ruff check .` **5990 处**（风格、简化、类型注解类为主）；CI 只把真 bug 级规则集（`E9,F63,F7,F82`）纳入闸门 —— **该规则集 2026-09-16 实测 0 处未过（All checks passed）**。原记录写的是"4858 处"与"3 处未过"，均已过期 |
| 9 | `check_principles.py` 未全过 | **2026-09-18 实测 10/12**。未过两条：**P6**（`core/world/firewall.py` 里一处 `"error": "internal"` —— 它是**写进隔离区日志的机器码**、不是给用户看的文案；判据按"字典 error 字段必须含中文"扫到了它，**这一条是判据口径偏严**，代码没动）、**P9**（版本号自洽：`neko_plugin/xiaojiao_install/README.md` 里引了一串**别的项目的版本号**，被判"查无此版"—— 同样是**非本项目文件**）。**上一轮两条红本轮已修**：P1（判据原来用 `utf-8` 读源码，**带 UTF-8 BOM 的 .py 被误判语法错** —— 解释器按 PEP 263 是认 BOM 的，已改成 `utf-8-sig`）、P8（`check_docs` 现在**不扫被 .gitignore 忽略的目录里的 .md**，`llama-swap/` 那份第三方 README 不再算本项目的错）。 |
