...
| 9 | `check_principles.py` 未全过 | **2026-09-16 实测 10/12**。原记录写的是"9/12，未过 P1/P6/P8"，**已过期** —— 现在未过的是 **P6**（面向用户的英文 error 文案，1 处 `core/world/firewall.py`）与 **P9**（版本号自洽：`neko_plugin` 的 README 引了一段含 `v3.13.13` 的真实响应片段，被判成"查无此版"）。**P1（ruff）与 P8（文档一致性）现已通过** |
