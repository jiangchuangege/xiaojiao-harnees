...
## 5.1 同一句话被写了很多遍（已修：跨轮去重）

**现象**：存量 17 条 formed 里，「摸到两个红球…」占了 **11 条**、「数字在眼前转…」占 **5 条** ——
同一句偏好被写了很多遍。
...
**真修**：`form()` 里加一道**跨轮去重** —— 跟**已存的 formed** 比，太像就不写：
...
| 决定 | 理由 |
|---|---|
| **复用 `SIMILAR = 0.75`** | 不新定第二条阈值 —— "同一句话"和"同一类心"用同一条线 |
| **相似就"不写"，不是"覆盖"** | 旧记录是**它当时说的那句话**，改它就是改写历史 |
| **存量 17 条不动** | 历史不去重、不删（同上：那是当时它说过的话） |
...
4. **跨轮去重也是载体算的**（复用 `SIMILAR = 0.75`）：它换一句话说同样的意思、
   相似度掉到线下，就会**再记一条** —— 去重不是分类，只是"太像就不重复写"。
   历史存量 17 条**不追溯**（如实标注：那批重复还在库里，没清）。
