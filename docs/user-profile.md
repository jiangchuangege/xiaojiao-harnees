...
单模型单机；自然对话里的长期稳定率**没有实测数据**。
（⚠️ 这里如实分清两种"没有"：**这一条只是"没做长测"**（条件够就能测，属待办）；
而"它有没有主观体验"那一类是**原则上不可验证**——两者不是一个性质，见 [`一条没人走过的路.md`](一条没人走过的路.md) 第四节。）
...
## 附六 · 闭环流量表（`core/profile_loop_meter.py`）

### 为什么要有它（这是被一次实测逼出来的）

附三那条闭环（载体给事实 → **模型判断要不要记** → 载体写画像 → 下次作为事实注入）
机制一直是通的，但「它到底有没有在积累」这件事，以前没有任何地方看得见。

2026-09-17 用户拿一张卡片问「这个问题你不是修过了吗」，顺着查下去才发现：
**环在转、里面没有东西在流动，而且它不会报错**。当天翻了一整天日志才数出来：

| 项 | 实测（2026-09-17 全天） |
|---|---|
| 第三步判断 | **73 次**（要记 8 / 不记 65） |
| 那 8 次「要记」记了什么 | **全是同一句**（测试流量反复喂的「用户 25 岁」）—— **只有 1 种** |
| 第四步注入 | **49 轮 / 129 条 / 不同 10 种** |
| 画像库当天新增 | **6 条**（其中至少 2 条是测试流量） |

也就是说：**真实对话里没有新东西进来**，注入端一直是那几样。
这种空转过去只有「人工翻日志」才能发现 —— 这就是这个模块存在的全部理由。

### 它记什么（只记账，不参与任何判断）

| 数 | 从哪来 |
|---|---|
| 判了要记 / 不记几条 | `xiaojiao_app.py` 的 `_profile_judge()`，判完之后各记一笔 |
| 注入了几轮 / 几条 / **不同几种** | 拼 system 那一段的画像注入处 |
| 画像库当天新增几条 | 读 `core/user_profile` 现算（排除迁移进来的与测试流量） |

落盘 `logs/psyche/profile_loop.jsonl`（一行一事件，只追加）。
**「不同几种」是关键数字**：它一直是 3 这种小数字，就说明没有新信息进来。

### 怎么看

```powershell
python core\profile_loop_meter.py              # 今天 + 最近 7 天的表（空转会在备注里点出来）
python core\profile_loop_meter.py --backfill   # 把 logs\xiaojiao.log 里已经发生的账补进来（同一天只补一次）
python core\profile_loop_meter.py --selftest   # 离线自测（20 项，写临时文件，不动真库）
```

应用里也会**自己报一行**（节流：最多 60 秒一句），长这样：

```
闭环流量·2026-09-17：判了 73（要记 8 / 不记 65）｜注入 50 轮 / 132 条 / 不同 11 种
  ｜画像库新增 6 条｜⚠️ 「要记」的全是同一条（1 种）：没有新的可积累信息
```

### 三个「空转」告警（写在日志里，不用等人去翻）

1. 判断 ≥ 10 次而要记的条数是 **0** → `环在空转`
2. **「要记」≥ 5 条但只有 1 种** → `「要记」的全是同一条` ← ⚠️ 这一条是 9-17 实测**补出来**的：
   那天账面上有 8 条「要记」，只按第 1 条判据**看不出空转**（它不是 0），
   但 8 条全是同一句 —— 「账面上有、实际没有新东西」这种空转必须也能被抓到。
3. 注入 ≥ 10 轮而**不同 ≤ 3 种** → `注入一直是那几种：没有新信息进来`

### 如实标注

1. **它只是仪表，不治空转。** 它把「有没有在积累」变成每天一行日志 ——
   真正让水流进来的是**真实对话**（9-17 那天的流量绝大部分是测试用例，所以判断里 65 次「不记」
   **大部分是对的**，不能拿这个数字去怪判据太严）。
2. **9-17 的数字是回填的**（`--backfill` 从 `logs/xiaojiao.log` 解析），回填事件标了
   `src=回填自日志`，与实时事件（`src=live`）分得开。
3. **装它的时候自测抓到两个真 bug**（都写进了代码注释）：① 以
   `python core/profile_loop_meter.py` 跑时 `sys.path` 不含仓库根 → 读画像库失败、
   表里一直显示「读不到」；② 回填解析那行用了 `｜` 的第二次出现当分隔，
   而那行**只有一个** `｜` → 「注入不同几种」被算成 0（恰好是看空转的关键数字）。

### 顺手搬走的一份遗留垃圾（同一轮做的，先备份再移）

`read_memory` 工具读的不是向量库，是仓库根目录一份**早期遗留**的记忆存盘
（72 行，最后写入 2026-08-28，2026-09-17 已搬进隔离区）。里面有那批模型退化乱码
（`亻命鸵次次次亻扑…` 重复 9 遍），用户 9-17 在界面上看见了它。
处理方式照本项目规矩 —— **不删文件，「清掉」= 移进隔离区**：

| 项 | 值 |
|---|---|
| 搬家 | 根目录那份 → `logs/quarantine/xiaojiao_memory.txt` |
| 备份 | `logs/backup_before_memory_quarantine/`（与搬走那份**字节级相同**） |
| 校验 | 搬家前后 sha256 **一致**（`5e2a33b8…`）；根目录那份已消失；从隔离区能原样读回（2159 字符） |
| 效果 | `read_memory` 从此返回「还没有任何记忆」 |
| 说明 | `logs/quarantine/README.md`（为什么在这、怎么还原、谁还在读它） |

**2026-09-18 补上了写侧那道闸（原来这里写的是"写侧仍然没有闸"）**：
`plugins/memory.py` 的 `save_memory` 现在先过 `_gate()` 再写，三道判据 ——
① 复用对话记忆那条写闸（`core/mem_filter.clean_answer`：工具原文 / 坏回复 / 占位符）；
② **完全重复不重复追加**（同一条内容已经写过就不再写第二遍）；
③ 明显不成话的（同一个字符连 5 次以上 / 整条没有任何中文或字母数字）。
**不写的时候说实话**：返回「这条**没有记**：<原因>」（原话照旧打印到日志），
**不假装"已记住"** —— 那是粉饰。
自测：`tools/test_memory_plugin_gate.py`（14 条，含"工具原文被拦""重复不写""复读被拦"）。
⚠️ 如实标注：闸门判据是**载体算的**（复用现成阈值，不新定标准）；
存量那 72 行遗留仍在隔离区、**没有回填重判**。
