...
## 附二 · 接入主流程：画像召回层进了 `xiaojiao_app.py`（**纯新增**）

`xiaojiao_recall.py` 已经接进对话主流程。**原有逻辑一行没删**（可核对：`git diff --numstat
xiaojiao_app.py` = `53 新增 / 0 删除`），旧的画像注入（`core/user_profile.render(8)`）**原样留着**。

| 时机 | 做什么 | 在哪 |
|---|---|---|
| 用户消息进来 → 拼 system | `recall(user_msg)` → `build_system(impressions)` → 拼进 sys_text | `agent_run` 里"关于用户"那一段的位置，紧挨原有画像注入（第 10315 行起），**放最后**（挨着生成） |
| 大脑回复**已经返回之后** | `remember_from_message(user_msg)`，**daemon 线程**，异常全吞 | `_record_interaction()`（`/api/chat` 与 `/api/chat/stream` 两个接口的**共同汇聚点**，一次接入两处都覆盖） |

- `recall()` **自带快慢分路**：触发词那一路命中 → **直接返回 1 条、不调模型**（零延迟）；
  没命中才跑完整 10 路血管投票（3 规则票 1.0 + 7 模型票 0.5；门槛：有规则票要 2 票、
  纯模型票要 4 票；取前 2，第 2 条要够第 1 条的 0.6 倍）。
- 召回为空 → `build_system()` 给无印象版，**正常走**，不当异常。
- 写入**不许阻塞用户**：判该不该记 → 生成画像 → 判重，全程 2~3 次模型调用，全在后台线程里。
- 开关：`xiaojiao_control.json` 的 `capabilities.profile_recall_write`（默认开）只关写入那一步。
- 日志能对账：`画像召回：命中 N 条 → …｜<注入的画像原文>`、`画像写入：已记一条｜…`。
  写入没发生时也会记一行（这一句没记 —— 但不区分"模型说不记/判重/没生成"，
  因为模块不改，排查交给下面的 `--why`）。

### 接入验收（5 条，真走 `/api/chat`）

| # | 用户说 | 期望 | 实际 | 判定 |
|---|---|---|---|---|
| 1 | 今天心情不好 | 召回 失恋 | 命中 1 条｜用户最近刚失恋，情绪低落 | ✅ |
| 2 | 推荐首歌听听 | 召回 周杰伦 | 命中 1 条｜用户喜欢听周杰伦的歌 | ✅ |
| 3 | 晚上想吃点啥 | 召回 不会做饭 | 命中 1 条｜用户不会做饭，经常点外卖 | ✅ |
| 4 | 今天天气真好 | 不召回任何印象 | 画像召回：空（按无印象版正常走） | ✅ |
| 5 | 我养了一只猫 | 回复后自动写入一条新画像 | 召回 ✅（"猫"命中）；**写入没有发生** | ❌ |

**第 5 条卡在哪，我查到了原始证据**（`python tools/test_profile_recall_integration.py --why "我养了一只猫"`）：
...
**是模块的第一道门判了"不记"，不是写入链没接上。** 证据：同一条话走 ②③ 都是通的
（能生成出画像、判重也放行）。接入侧也确有运行 —— 5 轮每一轮的日志里都有一行
`画像写入：这一句没记`，且都出现在回复返回**之后**（异步、没拖慢用户等待）。
按硬性要求「不许改 `xiaojiao_recall.py`」，我**没有动那道门**：要么改它的判据提示词，
要么等你拍板；两条路我都准备好了，说一声就动。

顺带一提：`--why` 这个排查模式是这轮加的 —— 因为 `remember_from_message()` 对
"不记 / 判重 / 没生成"三种情况**都返回 None**，光看返回值分不出是谁的问题，
而三种的修法完全不同（模型侧 vs 闸门 vs **载体解析**）。

> 与本文件"附"那一节的 `xiaojiao_profile.py` 的关系：那是**另一套**（写入链 + 10 路召回 + JSON 库），
> 单独能跑、已自测，但**没有接进主流程**。主流程现在用的是 `xiaojiao_recall.py` 这一套。
> 两套都在，谁都不许再删（只加不删）。
