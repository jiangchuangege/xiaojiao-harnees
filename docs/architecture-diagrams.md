...
| 图 | 对应 |
|---|---|
| **图 1–11** | [`design-philosophy.md`](design-philosophy.md) 前十三节 |
| **图 12–19** | 同一文件的**后半篇**（第十四至二十二节） |
| **图 20** | 后补的**载体改造·路径二四阶段**（内感受 → 视角 → 硬改 → 归属回流）—— 不对应设计哲学的某一节，对应的是真实提交 |
...
- **每一节都标实现状态**（已落地 / 部分落地 / 设计未落地）并指向真实代码路径 ——
  不标状态的架构图，等于把"想做"画成"做过"。
- **所有图都用 `flowchart` 语法，不用 mindmap**：本仓库的 `tools/check_mermaid.py` 会做
  括号 / 引号 / subgraph 配对校验，mindmap 不在校验范围内。用校验得到的语法，
  等于让每张图每次 CI 都被检查一遍。自检：`python tools/check_mermaid.py --all`。

> **为什么单独开一份图册而不是塞进 README**：README 是"第一眼"，读者扫两屏就该知道这是什么；
> 图册是"要细看时"才来的地方。混在一起的结果是 README 变成十几屏长图，第一眼就劝退。
...
## 图 21 · 兴趣积累闭环（记不记它自己判 · 载体只负责存 · 下次当事实摆回来）
...
**这张图要说的一句话**：记不记、记什么，**由它自己判断**（载体判就只能靠关键词，
而"随口问一次""帮别人问""不吃香菜和最爱吃香菜"三种必然误判）；
载体只做一问、一存、一摆 —— 然后把"下次答得更准"交给下一轮。

**如实标注（这一图有两条）**：

1. **最后一步在 4B 上没有走通。** 两遍实测（各 8 条）：事实进库 **7/8**、注入到 system **7/7**、
   **换新会话再问时真的用上只有 1/7 与 2/7**。链是通的，**"用它回答"没通** ——
   中途还把注入位置从 system 中段挪到最末跑了一遍，**没有可测量的提升（是噪声）**。
   数字与原始回答见 [`user-profile.md`](user-profile.md) 第 7 节。
2. **它自己的判断不稳定。** 同一句"我平时最爱看 NBA"，三次跑出三种结果
   （记下 / 输出自相矛盾 / 干净地判"不记"）。载体侧的判据已经堵到"不丢它的判断"，
   **但没法让 4B 的判断变稳**。

完整原理、判据与四个已修缺陷 → [`user-profile.md`](user-profile.md)
...
| 量的是什​么 | 结果 |
|---|---|
| 印象进没进 system（接线） | 5/5（含"今天天气真好"→ 空，按无印象版正常走） |
...
1. **有一条是"召不到"，不是"用不上"。** 问「晚上想点外卖，帮我推荐两个菜」时，
   两条链都没把「花生过敏」注进去（血管那条走了**快路**：触发词"外卖"命中就
   **立刻返回那 1 条、不再跑 10 路血管**）—— 回复于是没提过敏。
   **这是召回短路，不是模型不用。** 模块按你的要求没改，如实记在这里。
2. **样本小**：5 条问句、单模型（本地 4B），而且问题是为了"必须用到那条印象"而写的。
   它说明的是"**这条路走得通、有可核对的证据**"，不是"稳定命中率"。

完整原理与逐条原始回答 → [`user-profile.md`](user-profile.md) 附二。
