...
| ⑨ | 冲突仲裁 | `_resolve_llm_key`（密钥优先级）、`_history_summary_line`（多条大结果汇总） | 多个来源冲突时定优先级。**注意 `core/central/` 里不含仲裁逻辑**，它是黑板与事件总线 |
...
**边界。** 拆解的粒度是「一步」，不是完整计划树。把多步任务展开成有序计划属于**已决定不做**的方向（多智能体协作：与「模型是可替换火种」的前提冲突，永久划掉）。
