...
精排闸门由三个常量控制：`RERANK_GAP = 0.50`、`RERANK_MAX = 5`、`RERANK_MIN_HITS = 2`。只有候选不少于两条、且第一名与第二名分差小于 0.50 时才触发一次模型精排。分差足够大（top1 真的碾压）时才认定向量排序本身可信，不再多花一次模型往返。

> 2026-09-18 这条线由 `0.10` 提到 `0.50`：`0.10` 太容易过 —— 真实查询里 top1 领先 0.2 是家常便饭，于是绝大多数查询都被判成"排名不含糊 → 跳过精排"，**精排实际上很少跑**（实测日志：「3 条候选 → top1 领先 0.236，排名不含糊 → 跳过精排」），噪音直接进 system。代价是精排调用变多（每次约 400ms，见 `RERANK_GAP` 常量处的说明），这是**明知代价、选择精度**。
>
> 同一天还改了第二处：**判官不可用时不再"全部保留"** —— 改按 `THRESHOLD = 0.6` 卡一道，卡完若一条不剩仍保留 top1（不空手而归）。原因：大脑这条线不通时，原来的"全部放行"等于把噪音当事实（实测日志：「3 条候选 → 判官不可用，全部保留」，游泳 / 周杰伦照样进去）。这不是载体替模型判断 —— 判官在时照常问大脑，判官不在时载体手里只剩已算出的分数。**仍有一条没改**：`rerank()` 自身抛异常时最外层 `except` 依旧**全部放行**，如实标注。
...
处理方式由精排闸门负责：第一名与第二名分差小于 0.50 时触发一次模型精排，由模型判断方向。若精排没有触发，检查 `RERANK` 开关、`RERANK_GAP` 阈值，以及日志 `logs/memory_retrieval.log` 里的精排记录与时延分段。判官本身不可用时，结果按 `THRESHOLD = 0.6` 卡一道（至少留 top1），说明里会写明「判官不可用 → 按阈值 0.60 卡一道：保留 N/M 条」—— 看到这句说明就该去查大脑那条线了（没装 / 没起 / 报错）。
