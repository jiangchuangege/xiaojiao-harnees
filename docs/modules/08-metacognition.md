...
接入情况需要说清楚（**这一节在 2026-09-16 更正过：`self_rate()` 已接入主流程**）。

- **`self_rate()` 已接入 `agent_run`**，走**两级**：
  **第一级**是载体规则先判（不花模型算力），用 `_rule_selfrate()` 看本轮是否命中"不确定/不清楚/我查一下"
  这类措辞与是否调用过工具；
  **第二级**才在规则判不出来时让模型自评（`self_rate(..., llm_fn=..., context=_eyc_now())`）。
  日志里两种都能看见：
  `元认知自评：**载体规则**判为 C` 与 `元认知自评：规则判不出，交模型自评 rating=C`。
  随后 `route(rating)` 决定这一轮走哪条（`use_tool` 等）。
  > 早先本文写的是"主流程当前未调用它，改用规则近似" —— 那是**当时的实况**；
  > 现在两级都接了，更正为已接入。
- **`cross_check()` 已接入 `agent_run`（2026-09-18 接上）** —— 接在**答后**那一段（和"元认知标注"挨着）。
  三条硬规矩：
  ① **默认关**（`capabilities.cross_check`）：它要按 n 个角度各问一次模型，本地 4B 上那是成倍的等待，
  不能默认让每一轮都变慢；② **只挑"已经怀疑它"的那一轮跑**（自评 B/C，或回答里自己带了不确定措辞，
  见 `_should_cross_check()`），不做无差别全量；③ **判不了就如实说判不了**：
  `verdict=unknown`（没模型 / 样本不足）时**一个字都不加**，绝不把"没检查"写成"检查过、没问题"。
  判出 `conflict`（换个角度问对不上）时，给回答加一行说明并写明"请当参考、不要当结论"。
  自测钉的是**接线本身**：`tools/test_cross_check_wired.py`（17 项，含"该跑/不该跑"两头与它抓不住的边界）。
  > 早先本文写的是"`cross_check()` 仍然没有接入对话主流程（调用点只有自测）" —— 那是**当时的实况**：
  > 模块写得完整、自测全绿，**但主流程一次都没调用过它**。这种"模块是绿的、接线没接"的缺口，
  > 只有把"接线"本身钉住才不会再犯。现在更正为已接入（默认关）。
...
- `self_rate()` **已接入对话主流程**（两级：载体规则先判，判不出才交模型自评）；
  `cross_check()` **也接上了**（2026-09-18，接在答后、**默认关**、只在"已经怀疑它"的那一轮跑）。
  详见 4.1 节。
...
| 2026-09-18 | v1.0 | 交叉检查**接进主流程**（默认关、只在自评没把握的轮次跑、判不了就如实标 unknown）；4.1 与第 7 节同步更正 | 小焦项目 |
