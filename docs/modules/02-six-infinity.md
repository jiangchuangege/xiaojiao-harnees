...
| 本次实测结果 | 见 1.3 节的逐项清单 |
| 文档状态说明 | **2026-09-16 更正**：第 5 步的四个子项（工具调度强制路由、工具结果上下文隔离、结果校验、archify 工作流强制）**都已落地**，代码落点见 [`../six-infinity.md`](../six-infinity.md) 的 4.4.5 节。第 6 步的三项口径仍为设计目标 |
...
七个无限解决的问题是：当单次请求的 token 数是硬物理上限时，由载体在模型外面承担记忆存储、输入切片、输出续写、工具装载与痕迹隐藏，使总量与用户的感知不受单次上限约束，同时由第六项守住「每一次请求都装得下」这条底线。
...
| ④ | 工具无限 | 工具一个不删，载体只决定这一轮发多少条结构声明 | `_intent_tool_names`、`_plan_tools`、`_tool_index`、`plugins/` | **已落地**（含第 5 步四项，见 4.4.5） |
| ⑤ | 感知无限 | 用户看不到切片、循环、合并、检索等技术痕迹 | `on_progress` 不带片号、`api_chat_stream` 只推正文 | **已落地**（措辞统一为持续核对项） |
| ⑥ | 单次请求永不超 | 精确按需装配，保证每次请求都装得下 | `_estimate_tokens`、`_plan_tools`、`_fit_context`、`_max_context_tokens` | **已落地**；第 6 步的"超限告警"口径为设计目标 |
| ⑦ | 上下文无限 | 不扩 ctx，改装配：最近几轮留原文、更早压摘要、再往前按需检索 | `_fit_context`、`_history_digest`、`_max_context_tokens` | **已落地**，原理见 [`../context-infinity.md`](../context-infinity.md) |
...
七个无限处理的就是这两者之间的落差。前五项由载体在模型外面兜出来，不假装上限不存在；
...
1. 能力全部落在载体里。七个无限的任何一部分都不依赖模型特性，换火种后行为不变。
...
说明：这张图是全文的索引。七个无限是一条流水线上的六个环节，共用同一套载体代码。
...
工具无限的前四步已落地并有实测。**第 5 步的四项也已落地**（2026-09-16 更正；早先本文标的是"设计中、未落地"，属过期标注）。
一是工具调度强制路由：网址走抓取链，画图走画图链，搜索禁用功能词，查询走网络信息类工具，未识别则走闲聊
（代码落点：`xiaojiao_app.py` 的「工具选择顺序」表）。
二是工具结果上下文隔离：结果超过约 500 字时只把「前 200 字加工具名加状态码」写进持久历史，
完整结果仍对当次回答可用（用户拿到的 `answer` 一个字不动）。
三是工具结果校验：对照目标特征如域名与标题，不符则标记为可能幻觉。
四是工作流强制：画图前必须先读技能文档，交付前必须有校验步骤，缺失时由代码层补调
（`_ARCHIFY_GATE = "archify_validate"`）。
逐项代码落点见 [`../six-infinity.md`](../six-infinity.md) 的 4.4.5 节。

此外，工具结果的持久化**在历史里写入的是摘要**（上面第二项），
但大体积 JSON 与 HTML 的**语义压缩**仍属未来方向，未落地。
...
属于约定、需要后续步骤收口的部分是日志与报错文案的措辞统一。

**⚠️ 2026-09-16 更正**：本段早先写着「第 5 步与第 6 步的设计中包含『措辞统一』这一项…这一条为设计、未落地」。
现在要分开说：**第 5 步的四项机制已经落地**（见 5.4.6 节）；**仍然成立的只有一半** ——
「日志与报错文案的措辞统一」**尚未全量核对**，它是**持续核对项**，不是"机制没做"。
...
1. 七个无限里只有第六项站在物理上限这一侧，另外五项由载体在模型外围兜出来，不对物理上限作任何承诺。
2. 记忆无限的质量受限于字符级向量后端，精细语义区分度有限。
3. 输入无限的处理时间随片数线性增长，每片一次模型调用。
4. 输出无限的耗时随篇幅线性增长，按既有记录，50000 字约需 13 分钟起。
5. **工具无限第 5 步的四项已全部落地**（2026-09-16 更正），四项内容见 5.4.6 节。
6. 感知无限的机制已落地；**日志与报错文案的"措辞统一"仍是持续核对项**，未全量核对。
7. 单次请求永不超的第 6 步（超限告警口径）为设计、未落地，三项内容见 5.6.5 节。
...
- [`../six-infinity.md`](../six-infinity.md)：七个无限的原理、取舍、真实数字与已知问题，本文的主要上游文档。
- [`../six-infinity-diagrams/01-overview.md`](../six-infinity-diagrams/01-overview.md)：七个无限总览图与七张详细流程图。
- [`../design-philosophy.md`](../design-philosophy.md)：项目设计哲学，其中第二节、第七节、第十四节、第十五节与本文直接相关。
- [`../architecture-diagrams.md`](../architecture-diagrams.md)：图册版，其中图 10 的主题为七个无限。
...
| 日期 | 版本 | 变更 |
| --- | --- | --- |
| 2026-09-14 | v1.0 | 首次编写。建立七个无限的逐项说明、八张流程图、实测数字清单与未落地项标注；实测数据取自本机运行的六个验收脚本与体检脚本。 |
