...
`HealthHealer` **已接入主程序**（`xiaojiao_app.py` 里 `from core.health.heal import HealthHealer`，
并用 `HealthHealer(hooks=_health_hooks())` 实例化，见 7608/7613 行附近）。
它的默认实现遵循如实记账原则：**在没给回调的场合**（自测、单独调用），
切换火种会如实返回失败、后台整理返回 `False` 并说明没有可用的整理能力，**而不是假装完成**。
...
| 载体主动动作白名单 | 已落地，自测覆盖 |
| 会话隔离 | 已落地，自测覆盖 |
| 空闲后后台整理记忆 | 仅预防层调用与默认实现已落地。默认实现会尝试调用主程序的整理能力；主程序若要提供该能力需注入对应回调，未注入时实际返回失败并如实记账（不假装完成） |
| 睡眠与压力管理**在本模块内**没有实现 | **⚠️ 2026-09-16 更正：睡眠不是"未落地"，是落在别处了。** 预防层列出的四项里，体检、空闲整理、隔离在本模块；**睡眠另有完整实现**：`core/energy.py`（精力 0~1 的消耗与回升）、`core/heartbeat.py`（心跳不停 / 挂起 / 唤醒）、`xiaojiao_app.py` 的 `_sleep_all()` / `_wake_all()`，以及"**睡不睡由它自己写『睡：要 / 不要』**"那条链（见 [`../self-sleep.md`](../self-sleep.md)）。**压力管理**在载体侧有 `core/interoceptive.py` 的 `survival`（生存信号）与 `core/perspective.py` 的 `vigilance`，但**没有"健康预防层"意义上的压力管理** —— 这一半仍未落地 |
