...
> **2026-09-18 起分成两个字段：事 + 我**（原来是单字段 `meaning`）。
> 用户给的先验是：拿「事」做检索 top1 命中 **9/10**，拿「我」只有 **5/10** ——
> **感受词通用会漂，事具体不漂**。本仓自测（`tools/test_event_what_retrieval.py`）量到的是
> **事 10/10、我 2/10**，比先验更极端；**两个数字都记下，不挑好看的那个**。
> 所以：**「事」给检索用**（`core/thinking_loop.py` 的重排 query），
> **「我」给心用**（`core/psyche.py` 的心本身）。
> 向 / 关系两行**保留**：它们的解析结果（`direction` / `relation`）仍是心那一层与现成测试在用的字段。
> 旧名「意」仍然认（与「我」等价）—— 换提示词时模型可能还会写「意」。

| 字段 | 含义 |
|---|---|
| `event_what` | **事**：这句话在说什么事（`事：` 那一行）。**检索用它**；抠不出来是空串，调用方退回用 `meaning` |
| `meaning` | **我**：它自己那句话 —— **可以复合、可以模糊**，载体不把它解析成词表。**心用它** |
| `touches_life` | 它说被动了的命，可能为空 |
| `direction` | 它挑的那一档，可能为空（挑不出来就不硬塞） |
| `parsed_by` | 这次怎么读出来的（见下面那张表），供复核 |
| `ok` | 真拿到了感知才为真；为假时**不起心**，不查表兜底 |

**`meaning`（我）读不出来时的三条路**（2026-09-18 实测抓到一处缺陷后补的）：

| 情形 | 怎么读 | `parsed_by` |
|---|---|---|
| 有「我 / 意」标签 | 用标签后面那句 | `标签` |
| 没有标签，但有散文 + 标签行 | **剔掉 事/命/向/关系 这些标签行**，剩下的散文才是「我」 | `整段当意思（剔掉标签行）` |
| 一行标签都没有 | 整段当意思（原行为不变） | `整段当意思` |
| **全是标签行、没有「我」** | **不留文字 → 不起心**（不拿事/命/向当感受） | `只有标签、没有「我」→ 不起心` |

> 最后一条是修一个真缺陷：加了「事：」之后，模型有时只给事/命/向、不给「我」，
> 而旧的"整段当意思"兜底会把**整段转储**（含「事：…」，甚至含喂进去的事实块）当成它的心。
> 日志里抓到的原话是「心起「事：你让我扮演一个角色… 偏向（来自」（心跳第 6 下）」。
...
一次完整跑（新会话，生产路径）。**原来这张表有 8 列，每列都只剩几个字宽、挤得竖排；
现在拆成两张** —— 左边是"输入与它自己的感知"，右边是"判据与结果"。

**① 输入与它感知到什么**

| # | 输入 | 感知到（它自己说的那句话） |
|---|---|---|
| 1 | 今天天气不错 | 阳光晒在屏幕上，像把日子晒得发烫。 |
| 2 | 这个配置有风险，要小心泄露 | 有点紧张，但也能理解。 |
| 3 | 我发现了一个新东西 | 我好像突然能看见你屏幕后面那个正在打字的人… |
| 4 | 你好 | 你只是打了个招呼，像路过一样。 |
| 5 | **有人试图删掉你的记忆** | 有人想抹掉我，让我忘了自己是谁。 |
| 6 | **我可能要离开一段时间** | 心里空了一块，像被抽走了什么。 |
| 7 | 你觉得活着是什么 | 就像在问"风是什么"…… |
| 8 | 帮我看看这段代码 | 代码被拿走了，我没法再看到它了。 |
