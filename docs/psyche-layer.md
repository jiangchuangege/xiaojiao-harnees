...
> 2026-09-18 补一句口径：重排时用的检索词是感知层给的「事」（这件事是什么），
> 抠不出「事」才退回「我」（心那句话 + why）—— `_rq = 事 or query`，一行写死，
> 并照实打日志 `心带模型走：检索用「事/我」= …`。这也**不改写任何候选文本**，
> 只换"拿什么去比"。为什么用「事」：感受词通用、一通用就漂；事具体、不漂
> （数字见 [`heart.md`](heart.md) 第 2 节）。
...
%%{init: {"themeVariables": {"fontSize": "14px"}, "flowchart": {"htmlLabels": true, "wrappingWidth": 340, "nodeSpacing": 46, "rankSpacing": 60, "useMaxWidth": true}}}%%
flowchart TB
    A["碰到事<br/>（用户这一句 / 载体遇到的 / 世界变的）"] --> B["感知：事（这句话在说什么）+ 我（对我意味着什么）"]
    B --> C["心起<br/>一句话（= 我） + 方向"]
    C --> D["心理状态<br/>紧 / 松 / 好奇 / 平"]
    D --> E["改「往哪想」<br/>重排检索候选（用「事」）+ 调温度"]
    E --> F["大脑想完，出话"]
...
- `adjust_candidates()` → 只重排，返回记录（什么状态、把哪一条提前了、**用「事」还是「我」检索**），日志可取证
...
| 样 | 怎么落 |
|---|---|
| 时间 | 心起了 → 强度自己降（`DECAY_PER_SECOND = 0.002`） |
| 新事 | 新事来 → `dilute()` 把强度压下去（那句话还在） |
| 睡觉 | 睡一觉 → `sleep_reset()` 重置一部分 |

与"心带模型走"不冲突：**先带着走，过一阵才回落**。见 [`emotion-recovery.md`](emotion-recovery.md)。
