...
> 小焦（XiaoJiao）是一个以「载体优先架构」构建的本地 AI 助手，MIT 许可证开源。
> 它的基本主张是：**模型权重只是一个可替换的零件，助手的能力主要来自模型之外的框架代码。**

载体承担记忆检索、输入切片、输出续写与合并、上下文装配、工具注册与调度、结果校验、
状态管理、健康监测与人格表达；模型只承担单次推理。因此同一套载体接入小参数模型、
中等参数模型或任意 OpenAI 兼容接口，用户侧看到的都是同一个助手，
**更换模型不会丢失记忆、工具与人格设定**。

载体侧的**七项能力**——记忆、输入、输出、工具、感知、单次装配、上下文——共同保证
用户感知层面不设上限，同时**如实承认单次请求的 token 数是物理限制**。
项目全程本地运行，模型、记忆与会话均不离开本机，并在载体层**硬性拦截删除文件**的操作。
