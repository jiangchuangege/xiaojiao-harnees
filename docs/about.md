...
需要靠载体把任务拆开、多次执行、再拼起来，而不是指望单次前向完成。
...
| 超长输出 | 单次请求能生成多少就是多少 | 用户要 3000 字、5 万字时，只能分多次生成再拼接 |
...
| 全本地部署 | 只使用本机跑得动的模型 | 单次要靠载体拆步与循环来补，不能靠提示词堆 |
...
小焦不把模型输出异常当作「坏了」，而是当作「**生病了**」—— 所以它配了一个**医生**：
监测 → 诊断 → 治疗 → 病历 → 预防，五个环节各司其职
（`core/health/monitor.py`、`core/health/diagnose.py`、`core/health/heal.py`）。

| 环节 | 医生在这里做什么 |
|---|---|
| **监测** | 认 **18 类症状**：语言 4 类（复读、乱码、断句、语速突变）、逻辑 4 类、情绪 3 类、行为 4 类、生理 3 类（响应超时、显存告警、内存增长） |
| **诊断** | 四级分诊，按严重度决定动哪一档 |
| **治疗** | **一级** 静默重试与截断复读；**二级** 清理上下文并重置状态；**三级** 切换备用火种并回滚状态；**四级** 停止服务、保留现场、等待人工介入 |
| **病历** | 每级治疗写入 `logs/health/records.jsonl` |
| **预防** | 病历回灌成后续预警 |

它管的是**模型（火种）**的病。**载体**自己的病另有入口 —— **疼**与**四样命**
（记忆 / 连续 / 世界 / 关系），见 [`pain.md`](pain.md)。
两者不要混：**医生治火种，疼治载体。**
