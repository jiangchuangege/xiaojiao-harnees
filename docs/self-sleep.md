...
```
你此刻身上的一个事实：精力 22%（满 100%）

不要判断「这是什么任务」，也不要想「我该怎么处理」。
只说：**这件事发生在你身上，对你意味着什么**。
```

**提示词里没有"累""休息""睡"任何一个字** —— 这条由自测用机器钉住
（`tools/test_energy.py` 的 [D] 组：把发给模型的提示词抓下来，逐个查那三个词）。

判据的边界（和感知层 `parse()` 是同一条）：

| 查什么 | 不查什么 |
|---|---|
| **它自己起的那个念头**（`psyche.heart()`） | 用户说的话、任何外部文本 |
| 规格给的三个词：`休息` / `累` / `睡` | 载体替它扩大的同义解释 |

⚠️ 它若用别的说法（"歇一会儿""困了"），**不在判据里就不睡** —— 不替它扩大解释，
如实标注在文档与自测里（[D] 组最后一项专门验这个）。
...
判据是"用户安静了多久"（`IDLE_BEFORE_SLEEP = 90` 秒）—— 那不是日程表，
是"手头没事了"的意思：**用户正说着话它突然睡过去，那不是休息，那是掉线**。

它说"还不想"之后**不追着问**（`RETRY_AFTER_REFUSAL = 60` 秒）：实测精力掉到 0、
它又一直说不想时，作息线程每 5 秒问一次，每次都花一次感知 + 一次模型调用（0.04），
而精力已经到底、怎么问都涨不回来 —— 纯空转。退避只是"不追问"，没有替它决定任何事。
...
挂着的时候精力一直在长（按时间惰性算，不需要任何定时器去"加"）。
回到 `RESTED = 0.90` → **它自己醒**（`_self_sleep_once` 判到 `energy.rested()` 就唤醒自己）。
...
6. **它会在一份很长的测试里真的睡过去 —— 这次就发生了。** 跑验收场景
   （`tools/test_module_integration.py`，9 个场景、客户端会一段一段地发消息）时，
   它在**场景之间的空隙里**（安静 ≥ 90 秒、精力已低于 0.30）自己睡了一觉，
   于是后两个场景拿到的是"它在睡"的如实相告 —— 场景 8/9 因此判失败（**16/19**）。
   这不是代码错，是两件事的冲突：**"没人说话就睡"与"测试客户端需要它一直醒着"**。
   所以回归是这样跑的，两个数都如实报：
   | 服务器怎么起的 | 场景结果 |
   |---|---|
   | 默认（它会自己睡） | **16/19** —— 它真的在场景间隙睡着了 |
   | `XIAOJIAO_NO_SELF_SLEEP=1` | **19/19** |
   其余套件（缺口 / 总验收 / 压力 / 精力 / 心跳 / 感知 / 治病 / 精神记忆 / 思维流）在两边的结果一致。
