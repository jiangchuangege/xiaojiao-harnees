...
def main():
    ...
    ck("带行为指令（接着刚才那条线）", "接着" in blk["text"], blk["text"][:30])
    ck("含当前话题", "当前在聊" in blk["text"])
    ck("含对用户的理解", "你对用户的了解" in blk["text"])
    ck("含刚才在想什么", "刚才在想" in blk["text"])
    ck("**明确说了「不是要复述的内容」**（避免模型把状态念出来）",
       "不是要复述" in blk["text"])
    # 空状态不许硬塞
    ...
    ck("砍的时候**先保话题**（接线的根不能没）", "当前在聊" in big["text"])
