...
def main():
    ...
    #   第 3 处（2026-09-21 晚）：`_code_heal_answer` —— **用户只要看代码、或要的是网页/前端时，
    #   代码治病链压根不走**（那条链会去"生成 Python 并真跑它"，跑的时候红线命中，
    #   就把用户要的页面替换成一段告警；用户实测的原话：「我说写代码给我看展示到 web，没让他调用工具」）。
    #   ⚠️ 数字从 3 到 4 再到 5 是**判据用得更准**的结果，不是放宽：守的仍然是"**只有这几处**"，
    #   每加一处都必须在代码里写清为什么（这条注释就是那个"为什么"的镜像）。
    ck("**只有这几处**用它（工具表前置 / run_tool 两处 / 代码治病闸），别处一处都不许用",
       _calls == 5, _calls)
    ck("三处都在（前置 / 入口拦真写 / 代码治病闸）",
