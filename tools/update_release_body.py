...
- **（2026-09-21）本地大脑「模型名」对账**：`models[]` 里 `name` 是显示名、`model` 才是**要发给接口的 id**，
  两者混用时每次提问都是 `HTTP 404 · no router for requested model`，界面上还看不出为什么
  （实测：`brain.api.model` 写成显示名 `xiaojiao1.0-4B`，而服务认的路由键是 `xiaojiao`）。
  现在**保存设置 / 切预设 / 说话前**都会自动对一次账：换成服务真提供的那个 id 并写回配置，
  只试一次、探不到服务不算试过，日志里如实写"原来写的什么、为什么换"。
- **（2026-09-21）写文件撞上已存在的文件：拦截给了出路**：覆盖＝不可逆，红线照样拦；
  但拦完**由载体换个没被占用的新名字把内容写进去**（`a.txt` → `a_2.txt`），
  并明说"原文件一个字没动、内容写在新文件里" —— 原先只回一句"换个文件名或追加(mode='a')"，
  而 `write_file` 没有 mode 参数，模型照着做不了，会**连着几轮重试同一个动作**（实测踩到）。
  另外「写一段代码我看看」这类句子现在判成**要内容（当场说）**，不会再去建文件。
- **（2026-09-21）启动更稳**：日志轮转在"日志文件被别的进程占用"时不再刷 `--- Logging error ---`
  堆栈（继续追加、一条不丢、失败后退避）；启动前先探一次，**已经有一个小焦在跑就直接停下**
  （不会再让两份权重抢显存），要强开加 `--force`。
- **（2026-09-21 晚）「写段代码给我看看」就是写代码给我看**：用户的原话是
  「我说写代码给我看展示到 web，没让他调用工具」。现在：判为**要内容**时这一轮的工具表里
  **就没有 `write_file`**（看不到就不会去点；`run_tool` 那道闸照旧保留，因为 4B 会硬编工具名）；
  真去调会被拦下，且**只把内容端出来、不弹「⚠️ 安全红线」告警横幅**。
  顺带修掉一个大坑：这条拦截原来**没带载体标记**，被健康系统当成"模型乱调工具"→ 诊断 HEAVY →
  **三级治疗把大脑切到备用火种** → 去慢盘重载 4GB → 一轮十分钟不动（实测）。
  实测：**「写一段 Python 快排给我看看」→ 0 次工具调用、25 秒、代码直接贴在对话里**。
- **（2026-09-21 晚）「写个页面写一半就断」的真凶是预设里的 `max_tokens: 512`**：
  512 token 装不下一个页面。现在**要长输出的请求（写代码/网页/长文/完整…）至少给 2048**
  （你配得更大就听你的）—— 写代码和闲聊用不同额度，预设里那个 512 是给闲聊调语气的。
  （截断后"自动续写"那一段这次**没验成**，如实写在 CHANGELOG 里，不假装它好了。）
- **（2026-09-21 深夜）删除禁区**按用户要求**默认关掉**（代码一行没删、一开就回来）：
  用户的原话是「我建议删掉，你这个误伤源太 tm 多了」——他的实测场景是**让它写代码看看**，
  而「代码治病」那条链会**把生成的代码真跑一遍去验证**，跑的时候红线把代码里的 `> *`、`$(...)`
  读成"删除/覆盖动作"，于是**把用户要的整个页面替换成了一段告警**。
  现在：那条链**不再对"看代码 / 网页前端"类请求动手**；删除禁区整体默认关
  （想开回来：控制文件 `capabilities.no_delete_guard: true`，或环境变量 `XIAOJIAO_NO_DELETE=1`）。
  **代价写在 CHANGELOG 最显眼处：关掉之后它真的能删你的文件，没有回收站、不可逆。**
- **（2026-09-21 深夜）裸内容会自动包成正常代码块**：模型把语言名写进"准备写文件"的内容里时，
  界面上会出现一条平铺的正文、第一行孤零零一个 `html`；现在首行的裸语言名会被认成语言标记、
  内容没有围栏就补上，用户看到的是**正常的代码块**（已有围栏的不会重复包一层）。
